...
#ifdef VULKANSC  // Vulkan SC does not support sparse resources
...
#ifndef VULKANSC  // Vulkan SC does not support vkFreeMemory
...
#ifndef VULKANSC  // Vulkan SC does not support vkDestroyCommandPool
...
#ifndef VULKANSC  // Vulkan SC does not support Android HW buffers
...
#ifndef VULKANSC  // Vulkan SC does not support vkDestroySwapchainKHR
...
#ifdef VULKANSC
    auto *features_sc10 = vku::FindStructInPNextChain<VkPhysicalDeviceVulkanSC10Features>(pFeatures->pNext);
    if (features_sc10) {
        features_sc10->shaderAtomicInstructions = VK_TRUE;
    }

    auto *ycbr_conv_features = vku::FindStructInPNextChain<VkPhysicalDeviceSamplerYcbcrConversionFeatures>(pFeatures->pNext);
...
#ifdef VULKANSC
    auto *props_sc10 = vku::FindStructInPNextChain<VkPhysicalDeviceVulkanSC10Properties>(pProperties->pNext);
    if (props_sc10) {
...
#ifndef VULKANSC  // Vulkan SC does not support Vulkan 1.3 APIs
...
#ifndef VULKANSC  // Vulkan SC does not support ray tracing
...
#ifndef VULKANSC  // Vulkan SC does not support these extensions
...
#ifdef VULKANSC
        std::strncpy(driver_properties->driverName, "Vulkan SC Mock Device", VK_MAX_DRIVER_NAME_SIZE);
#else
...
#ifndef VULKANSC  // Vulkan SC does not support host image copies
...
#ifdef VULKANSC  // Vulkan SC does not support video
        if (*pQueueFamilyPropertyCount > 1) {
