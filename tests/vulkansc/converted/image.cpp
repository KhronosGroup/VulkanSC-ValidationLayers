...
TEST_F(NegativeImage, ImageSampleWeightedInstructionButInvalidFormatFeatures) {
    TEST_DESCRIPTION(
        "Try to launch a compute pass with an OpImageSampleWeightedQCOM instruction, while "
    ...
    compute_pipe.descriptor_set_.WriteDescriptorImageInfo(1, weight_image_view, nullptr,
...
TEST_F(NegativeImage, ImageBoxFilterInstructionButInvalidFormatFeatures) {
    TEST_DESCRIPTION(
        "Try to launch a compute pass with an OpImageBoxFilterQCOM instruction, while "
        "the used image views have the invalid format features.");
    SetTargetApiVersion(VK_API_VERSION_1_3);
    AddRequiredExtensions(VK_QCOM_IMAGE_PROCESSING_EXTENSION_NAME);
    ...
    compute_pipe.cs_ =
        VkShaderObj{*m_device, cs_source, VK_SHADER_STAGE_COMPUTE_BIT, SPV_ENV_VULKAN_1_3, SPV_SOURCE_GLSL, &specialization_info};
    compute_pipe.dsl_bindings_ = {{0, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1, VK_SHADER_STAGE_ALL, nullptr},
...
TEST_F(NegativeImage, ImageSampleImplicitInstructionButInvalidSamplerFlags) {
    TEST_DESCRIPTION(
        "Try to launch a compute pass with an OpImageSampleImplicitLod instruction, while "
...
TEST_F(NegativeImage, ImageBlockMatchInstructionButInvalidFormatFeatures) {
    TEST_DESCRIPTION(
        "Try to launch a compute pass with an OpImageBlockMatchSADQCOM instruction and "
    ...
    compute_pipe.descriptor_set_.WriteDescriptorImageInfo(1, reference_image_view, nullptr,
...
TEST_F(NegativeImage, ImageBlockMatchWindowInstructionButInvalidFormatFeatures) {
    TEST_DESCRIPTION(
        "Try to launch a compute pass with an OpImageBlockMatchWindowSSDQCOM instruction and "
    ...
    CreateComputePipelineHelper compute_pipe{*this};
    constexpr std::array<VkSpecializationMapEntry, 2> map_entries{VkSpecializationMapEntry{0, 0, sizeof(uint32_t)},
                                                                  VkSpecializationMapEntry{1, sizeof(uint32_t), sizeof(uint32_t)}};
    const std::array<uint32_t, 2> block_sizes{
        std::min(8u, image_processing_props.maxBlockMatchRegion.width),
        std::min(8u, image_processing_props.maxBlockMatchRegion.height),
...
TEST_F(NegativeImage, ImageBlockMatchWindowWithClampToEdgeButFeatureNotEnabled) {
    TEST_DESCRIPTION(
        "Try to use OpImageBlockMatchWindow*QCOM with CLAMP_TO_EDGE address mode, "
