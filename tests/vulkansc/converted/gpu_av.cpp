...
TEST_F(NegativeGpuAV, DISABLED_YcbcrDrawFetchIndexed) {
    ...
    const char vsSource[] = R"glsl(
        ...
        layout (set = 0, binding = 1) uniform UBO { uint index; }; // makes dynamic and not caught in core check
        void main() {
...
// Not supported in Vulkan SC: GPU AV
TEST_F(NegativeGpuAV, DISABLED_YcbcrDrawFetchNonArrayPartiallyBound) {
    TEST_DESCRIPTION("Do OpImageFetch on a Ycbcr COMBINED_IMAGE_SAMPLER.");
    SetTargetApiVersion(VK_API_VERSION_1_2);
    AddRequiredFeature(vkt::Feature::samplerYcbcrConversion);
...
// Not supported in Vulkan SC: GPU AV
TEST_F(NegativeGpuAV, DISABLED_NonMultisampleMismatchWithPipeline) {
...
// Not supported in Vulkan SC: GPU AV
TEST_F(NegativeGpuAV, DISABLED_DISABLED_AliasImageMultisample) {
