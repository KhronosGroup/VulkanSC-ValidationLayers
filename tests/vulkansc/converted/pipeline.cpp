...
// This test case are replaced with Vulkan SC specific ones
TEST_F(NegativePipeline, DISABLED_MissingEntrypoint2) {
    RETURN_IF_SKIP(Init());
...
%not_this = OpFunction %void None %4
  %l2 = OpLabel
    ...
    CreateComputePipelineHelper pipe(*this);
    pipe.cs_ = VkShaderObj(this, shader_source, VK_SHADER_STAGE_COMPUTE_BIT, SPV_ENV_VULKAN_1_0, SPV_SOURCE_ASM, nullptr, "main");
    m_errorMonitor->SetDesiredError("VUID-VkPipelineShaderStageCreateInfo-pName-00707");
    pipe.CreateComputePipeline();
    m_errorMonitor->VerifyFound();
}
...
// This test case are replaced with Vulkan SC specific ones
TEST_F(NegativePipeline, DISABLED_NullStagepNameCompute) {
    TEST_DESCRIPTION("Test that an error is produced for a stage with a null pName pointer");
    RETURN_IF_SKIP(Init());
...
// Not supported in Vulkan SC: derivative pipelines
TEST_F(NegativePipeline, DISABLED_CreateComputePipelineWithBadBasePointer) {
...
// Not supported in Vulkan SC: derivative pipelines
TEST_F(NegativePipeline, DISABLED_CreateComputePipelineWithDerivatives) {
