...
TEST_F(PositivePipelineTopology, PointSizeWriteInFunction) {
    ...
    VkShaderObj ps(*m_device, kFragmentMinimalGlsl, VK_SHADER_STAGE_FRAGMENT_BIT);
    {
...
TEST_F(PositivePipelineTopology, PointSizeGeomShaderSuccess) {
    ...
    VkShaderObj vs(*m_device, kVertexPointSizeGlsl, VK_SHADER_STAGE_VERTEX_BIT);
    VkShaderObj gs(*m_device, kGeometryPointSizeGlsl, VK_SHADER_STAGE_GEOMETRY_BIT);
    VkShaderObj ps(*m_device, kFragmentMinimalGlsl, VK_SHADER_STAGE_FRAGMENT_BIT);
...
TEST_F(PositivePipelineTopology, LoosePointSizeWrite) {
   ...
   %_ptr_Private__arr_v2float_uint_3 = OpTypePointer Private %_arr_v2float_uint_3
    ...
    // Create VS declaring PointSize and write to it in a function call.
    VkShaderObj vs(*m_device, LoosePointSizeWrite, VK_SHADER_STAGE_VERTEX_BIT, SPV_ENV_VULKAN_1_0, SPV_SOURCE_ASM);
    VkShaderObj ps(*m_device, kFragmentMinimalGlsl, VK_SHADER_STAGE_FRAGMENT_BIT);
...
TEST_F(PositivePipelineTopology, PointSizeDynamicAndUnrestricted) {
    TEST_DESCRIPTION(
...
// Not supported in Vulkan SC: VK_EXT_extended_dynamic_state3
TEST_F(PositivePipelineTopology, DISABLED_PatchListTopology) {
    AddRequiredExtensions(VK_EXT_EXTENDED_DYNAMIC_STATE_EXTENSION_NAME);
