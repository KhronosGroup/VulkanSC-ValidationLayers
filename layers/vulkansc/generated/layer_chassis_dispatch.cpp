    ...
    while (cur_pnext != nullptr) {
        ...
        switch (header->sType) {
            ...
            case VK_STRUCTURE_TYPE_SEMAPHORE_SCI_SYNC_CREATE_INFO_NV: {
                auto* safe_struct = reinterpret_cast<vku::safe_VkSemaphoreSciSyncCreateInfoNV*>(cur_pnext);
...
VkResult DispatchEnumerateDeviceExtensionProperties(VkPhysicalDevice physicalDevice, const char* pLayerName,
                                                    uint32_t* pPropertyCount, VkExtensionProperties* pProperties) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);

    VkResult result = layer_data->instance_dispatch_table.EnumerateDeviceExtensionProperties(physicalDevice, pLayerName,
...
VkResult DispatchAllocateMemory(VkDevice device, const VkMemoryAllocateInfo* pAllocateInfo, const VkAllocationCallbacks* pAllocator,
    ...
    vku::safe_VkMemoryAllocateInfo* local_pAllocateInfo = nullptr;
    {
        if (pAllocateInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pAllocateInfo->pNext);
        }
    }
...
VkResult DispatchCreateSemaphore(VkDevice device, const VkSemaphoreCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator,
    ...
    vku::safe_VkSemaphoreCreateInfo var_local_pCreateInfo;
    vku::safe_VkSemaphoreCreateInfo* local_pCreateInfo = nullptr;
    {
        if (pCreateInfo) {
    ...
    VkResult result = layer_data->device_dispatch_table.CreateSemaphore(device, (const VkSemaphoreCreateInfo*)local_pCreateInfo,
...
void DispatchCmdBeginRenderPass(VkCommandBuffer commandBuffer, const VkRenderPassBeginInfo* pRenderPassBegin,
    ...
    vku::safe_VkRenderPassBeginInfo* local_pRenderPassBegin = nullptr;
    {
        if (pRenderPassBegin) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pRenderPassBegin->pNext);
        }
    }
...
VkResult DispatchSetPrivateData(VkDevice device, VkObjectType objectType, uint64_t objectHandle, VkPrivateDataSlot privateDataSlot,
                                uint64_t data) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.SetPrivateData(device, objectType, objectHandle, privateDataSlot, data);
    {
...
void DispatchGetPrivateData(VkDevice device, VkObjectType objectType, uint64_t objectHandle, VkPrivateDataSlot privateDataSlot,
                            uint64_t* pData) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.GetPrivateData(device, objectType, objectHandle, privateDataSlot, pData);
    {
        if (NotDispatchableHandle(objectType)) {
            objectHandle = layer_data->Unwrap(objectHandle);
        }
...
void DispatchCmdBeginRendering(VkCommandBuffer commandBuffer, const VkRenderingInfo* pRenderingInfo) {
    ...
    vku::safe_VkRenderingInfo* local_pRenderingInfo = nullptr;
    {
        if (pRenderingInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pRenderingInfo->pNext);
        }
    }
...
void DispatchGetCommandPoolMemoryConsumption(VkDevice device, VkCommandPool commandPool, VkCommandBuffer commandBuffer,
                                             VkCommandPoolMemoryConsumption* pConsumption) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.GetCommandPoolMemoryConsumption(device, commandPool, commandBuffer, pConsumption);
    { commandPool = layer_data->Unwrap(commandPool); }
    layer_data->device_dispatch_table.GetCommandPoolMemoryConsumption(device, commandPool, commandBuffer, pConsumption);
}

VkResult DispatchGetFaultData(VkDevice device, VkFaultQueryBehavior faultQueryBehavior, VkBool32* pUnrecordedFaults,
                              uint32_t* pFaultCount, VkFaultData* pFaults) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);

    VkResult result =
        layer_data->device_dispatch_table.GetFaultData(device, faultQueryBehavior, pUnrecordedFaults, pFaultCount, pFaults);
...
VkResult DispatchCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo,
    ...
    VkResult result = layer_data->device_dispatch_table.CreateSwapchainKHR(
        ...
        if (pCreateInfos) {
            ...
            for (uint32_t index0 = 0; index0 < swapchainCount; ++index0) {
                ...
                if (pCreateInfos[index0].surface) {
                    local_pCreateInfos[index0].surface = layer_data->Unwrap(pCreateInfos[index0].surface);
                }
                if (pCreateInfos[index0].oldSwapchain) {
                    local_pCreateInfos[index0].oldSwapchain = layer_data->Unwrap(pCreateInfos[index0].oldSwapchain);
                }
            }
        }
    }
    VkResult result = layer_data->device_dispatch_table.CreateSharedSwapchainsKHR(
...
void DispatchCmdDecodeVideoKHR(VkCommandBuffer commandBuffer, const VkVideoDecodeInfoKHR* pDecodeInfo) {
    ...
    vku::safe_VkVideoDecodeInfoKHR* local_pDecodeInfo = nullptr;
    {
        if (pDecodeInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pDecodeInfo->pNext);
        }
    }
...
void DispatchCmdEncodeVideoKHR(VkCommandBuffer commandBuffer, const VkVideoEncodeInfoKHR* pEncodeInfo) {
    ...
    vku::safe_VkVideoEncodeInfoKHR* local_pEncodeInfo = nullptr;
    {
        if (pEncodeInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pEncodeInfo->pNext);
        }
    }
    layer_data->device_dispatch_table.CmdEncodeVideoKHR(commandBuffer, (const VkVideoEncodeInfoKHR*)local_pEncodeInfo);
}

void DispatchCmdRefreshObjectsKHR(VkCommandBuffer commandBuffer, const VkRefreshObjectListKHR* pRefreshObjects) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(commandBuffer), layer_data_map);

    layer_data->device_dispatch_table.CmdRefreshObjectsKHR(commandBuffer, pRefreshObjects);
}

VkResult DispatchGetPhysicalDeviceRefreshableObjectTypesKHR(VkPhysicalDevice physicalDevice, uint32_t* pRefreshableObjectTypeCount,
                                                            VkObjectType* pRefreshableObjectTypes) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);

    VkResult result = layer_data->instance_dispatch_table.GetPhysicalDeviceRefreshableObjectTypesKHR(
...
void DispatchCmdBindDescriptorSets2KHR(VkCommandBuffer commandBuffer, const VkBindDescriptorSetsInfoKHR* pBindDescriptorSetsInfo) {
    ...
    vku::safe_VkBindDescriptorSetsInfoKHR* local_pBindDescriptorSetsInfo = nullptr;
    {
        if (pBindDescriptorSetsInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pBindDescriptorSetsInfo->pNext);
        }
    }
...
void DispatchCmdPushConstants2KHR(VkCommandBuffer commandBuffer, const VkPushConstantsInfoKHR* pPushConstantsInfo) {
    ...
    vku::safe_VkPushConstantsInfoKHR* local_pPushConstantsInfo = nullptr;
    {
        if (pPushConstantsInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pPushConstantsInfo->pNext);
        }
    }
...
void DispatchCmdPushDescriptorSet2KHR(VkCommandBuffer commandBuffer, const VkPushDescriptorSetInfoKHR* pPushDescriptorSetInfo) {
    ...
    vku::safe_VkPushDescriptorSetInfoKHR* local_pPushDescriptorSetInfo = nullptr;
    {
        if (pPushDescriptorSetInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pPushDescriptorSetInfo->pNext);
        }
    }
...
void DispatchCmdSetDescriptorBufferOffsets2EXT(VkCommandBuffer commandBuffer,
    ...
    vku::safe_VkSetDescriptorBufferOffsetsInfoEXT* local_pSetDescriptorBufferOffsetsInfo = nullptr;
    {
        if (pSetDescriptorBufferOffsetsInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pSetDescriptorBufferOffsetsInfo->pNext);
        }
    }
...
void DispatchCmdBindDescriptorBufferEmbeddedSamplers2EXT(
    ...
    vku::safe_VkBindDescriptorBufferEmbeddedSamplersInfoEXT* local_pBindDescriptorBufferEmbeddedSamplersInfo = nullptr;
    {
        if (pBindDescriptorBufferEmbeddedSamplersInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pBindDescriptorBufferEmbeddedSamplersInfo->pNext);
        }
    }
...
VkResult DispatchGetRandROutputDisplayEXT(VkPhysicalDevice physicalDevice, Display* dpy, RROutput rrOutput,
    ...
    if (VK_SUCCESS == result) {
        *pDisplay = layer_data->MaybeWrapDisplay(*pDisplay);
    }
    return result;
}
...
void DispatchCmdInitializeGraphScratchMemoryAMDX(VkCommandBuffer commandBuffer, VkPipeline executionGraph, VkDeviceAddress scratch,
                                                 VkDeviceSize scratchSize) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(commandBuffer), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.CmdInitializeGraphScratchMemoryAMDX(commandBuffer, executionGraph, scratch,
                                                                                     scratchSize);
    { executionGraph = layer_data->Unwrap(executionGraph); }
    layer_data->device_dispatch_table.CmdInitializeGraphScratchMemoryAMDX(commandBuffer, executionGraph, scratch, scratchSize);
}

void DispatchCmdDispatchGraphAMDX(VkCommandBuffer commandBuffer, VkDeviceAddress scratch, VkDeviceSize scratchSize,
                                  const VkDispatchGraphCountInfoAMDX* pCountInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(commandBuffer), layer_data_map);

    layer_data->device_dispatch_table.CmdDispatchGraphAMDX(commandBuffer, scratch, scratchSize, pCountInfo);
}

void DispatchCmdDispatchGraphIndirectAMDX(VkCommandBuffer commandBuffer, VkDeviceAddress scratch, VkDeviceSize scratchSize,
                                          const VkDispatchGraphCountInfoAMDX* pCountInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(commandBuffer), layer_data_map);

    layer_data->device_dispatch_table.CmdDispatchGraphIndirectAMDX(commandBuffer, scratch, scratchSize, pCountInfo);
}

void DispatchCmdDispatchGraphIndirectCountAMDX(VkCommandBuffer commandBuffer, VkDeviceAddress scratch, VkDeviceSize scratchSize,
                                               VkDeviceAddress countInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(commandBuffer), layer_data_map);

    layer_data->device_dispatch_table.CmdDispatchGraphIndirectCountAMDX(commandBuffer, scratch, scratchSize, countInfo);
}
#endif  // VK_ENABLE_BETA_EXTENSIONS
...
VkResult DispatchGetDrmDisplayEXT(VkPhysicalDevice physicalDevice, int32_t drmFd, uint32_t connectorId, VkDisplayKHR* display) {
    ...
    if (VK_SUCCESS == result) {
        *display = layer_data->MaybeWrapDisplay(*display);
    }
    return result;
}
...
VkResult DispatchSetPrivateDataEXT(VkDevice device, VkObjectType objectType, uint64_t objectHandle,
                                   VkPrivateDataSlot privateDataSlot, uint64_t data) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.SetPrivateDataEXT(device, objectType, objectHandle, privateDataSlot, data);
    {
        if (NotDispatchableHandle(objectType)) {
            objectHandle = layer_data->Unwrap(objectHandle);
        }
...
void DispatchGetPrivateDataEXT(VkDevice device, VkObjectType objectType, uint64_t objectHandle, VkPrivateDataSlot privateDataSlot,
    ...
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.GetPrivateDataEXT(device, objectType, objectHandle, privateDataSlot, pData);
    {
        if (NotDispatchableHandle(objectType)) {
            objectHandle = layer_data->Unwrap(objectHandle);
        }
...
VkResult DispatchGetFenceSciSyncFenceNV(VkDevice device, const VkFenceGetSciSyncInfoNV* pGetSciSyncHandleInfo, void* pHandle) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.GetFenceSciSyncFenceNV(device, pGetSciSyncHandleInfo, pHandle);
    vku::safe_VkFenceGetSciSyncInfoNV var_local_pGetSciSyncHandleInfo;
    vku::safe_VkFenceGetSciSyncInfoNV* local_pGetSciSyncHandleInfo = nullptr;
    {
    ...
    VkResult result = layer_data->device_dispatch_table.GetFenceSciSyncFenceNV(
        device, (const VkFenceGetSciSyncInfoNV*)local_pGetSciSyncHandleInfo, pHandle);

    return result;
}

VkResult DispatchGetFenceSciSyncObjNV(VkDevice device, const VkFenceGetSciSyncInfoNV* pGetSciSyncHandleInfo, void* pHandle) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    ...
    VkResult result = layer_data->device_dispatch_table.GetFenceSciSyncObjNV(
        device, (const VkFenceGetSciSyncInfoNV*)local_pGetSciSyncHandleInfo, pHandle);

    return result;
}

VkResult DispatchImportFenceSciSyncFenceNV(VkDevice device, const VkImportFenceSciSyncInfoNV* pImportFenceSciSyncInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.ImportFenceSciSyncFenceNV(device, pImportFenceSciSyncInfo);
    vku::safe_VkImportFenceSciSyncInfoNV var_local_pImportFenceSciSyncInfo;
    vku::safe_VkImportFenceSciSyncInfoNV* local_pImportFenceSciSyncInfo = nullptr;
    {
    ...
    VkResult result = layer_data->device_dispatch_table.ImportFenceSciSyncFenceNV(
        device, (const VkImportFenceSciSyncInfoNV*)local_pImportFenceSciSyncInfo);

    return result;
}

VkResult DispatchImportFenceSciSyncObjNV(VkDevice device, const VkImportFenceSciSyncInfoNV* pImportFenceSciSyncInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    ...
    VkResult result = layer_data->device_dispatch_table.ImportFenceSciSyncObjNV(
        device, (const VkImportFenceSciSyncInfoNV*)local_pImportFenceSciSyncInfo);

    return result;
}

VkResult DispatchGetPhysicalDeviceSciSyncAttributesNV(VkPhysicalDevice physicalDevice,
                                                      const VkSciSyncAttributesInfoNV* pSciSyncAttributesInfo,
                                                      NvSciSyncAttrList pAttributes) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);

    VkResult result = layer_data->instance_dispatch_table.GetPhysicalDeviceSciSyncAttributesNV(physicalDevice,
                                                                                               pSciSyncAttributesInfo, pAttributes);

    return result;
}

VkResult DispatchGetSemaphoreSciSyncObjNV(VkDevice device, const VkSemaphoreGetSciSyncInfoNV* pGetSciSyncInfo, void* pHandle) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.GetSemaphoreSciSyncObjNV(device, pGetSciSyncInfo, pHandle);
    vku::safe_VkSemaphoreGetSciSyncInfoNV var_local_pGetSciSyncInfo;
    vku::safe_VkSemaphoreGetSciSyncInfoNV* local_pGetSciSyncInfo = nullptr;
    {
        if (pGetSciSyncInfo) {
    ...
    VkResult result = layer_data->device_dispatch_table.GetSemaphoreSciSyncObjNV(
        device, (const VkSemaphoreGetSciSyncInfoNV*)local_pGetSciSyncInfo, pHandle);

    return result;
}

VkResult DispatchImportSemaphoreSciSyncObjNV(VkDevice device, const VkImportSemaphoreSciSyncInfoNV* pImportSemaphoreSciSyncInfo) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.ImportSemaphoreSciSyncObjNV(device, pImportSemaphoreSciSyncInfo);
    vku::safe_VkImportSemaphoreSciSyncInfoNV var_local_pImportSemaphoreSciSyncInfo;
    vku::safe_VkImportSemaphoreSciSyncInfoNV* local_pImportSemaphoreSciSyncInfo = nullptr;
    {
    ...
    VkResult result = layer_data->device_dispatch_table.ImportSemaphoreSciSyncObjNV(
        device, (const VkImportSemaphoreSciSyncInfoNV*)local_pImportSemaphoreSciSyncInfo);

    return result;
}

VkResult DispatchGetMemorySciBufNV(VkDevice device, const VkMemoryGetSciBufInfoNV* pGetSciBufInfo, NvSciBufObj* pHandle) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.GetMemorySciBufNV(device, pGetSciBufInfo, pHandle);
    vku::safe_VkMemoryGetSciBufInfoNV var_local_pGetSciBufInfo;
    vku::safe_VkMemoryGetSciBufInfoNV* local_pGetSciBufInfo = nullptr;
    {
        if (pGetSciBufInfo) {
    ...
    VkResult result =
        layer_data->device_dispatch_table.GetMemorySciBufNV(device, (const VkMemoryGetSciBufInfoNV*)local_pGetSciBufInfo, pHandle);

    return result;
}

VkResult DispatchGetPhysicalDeviceExternalMemorySciBufPropertiesNV(VkPhysicalDevice physicalDevice,
                                                                   VkExternalMemoryHandleTypeFlagBits handleType,
                                                                   NvSciBufObj handle,
                                                                   VkMemorySciBufPropertiesNV* pMemorySciBufProperties) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);

    VkResult result = layer_data->instance_dispatch_table.GetPhysicalDeviceExternalMemorySciBufPropertiesNV(
        physicalDevice, handleType, handle, pMemorySciBufProperties);

    return result;
}

VkResult DispatchGetPhysicalDeviceSciBufAttributesNV(VkPhysicalDevice physicalDevice, NvSciBufAttrList pAttributes) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);
...
void DispatchGetPipelineIndirectMemoryRequirementsNV(VkDevice device, const VkComputePipelineCreateInfo* pCreateInfo,
    ...
    vku::safe_VkComputePipelineCreateInfo* local_pCreateInfo = nullptr;
    {
        if (pCreateInfo) {
            ...
            UnwrapPnextChainHandles(layer_data, local_pCreateInfo->stage.pNext);
...
VkResult DispatchCreateSemaphoreSciSyncPoolNV(VkDevice device, const VkSemaphoreSciSyncPoolCreateInfoNV* pCreateInfo,
                                              const VkAllocationCallbacks* pAllocator, VkSemaphoreSciSyncPoolNV* pSemaphorePool) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles)
        return layer_data->device_dispatch_table.CreateSemaphoreSciSyncPoolNV(device, pCreateInfo, pAllocator, pSemaphorePool);

    VkResult result =
        layer_data->device_dispatch_table.CreateSemaphoreSciSyncPoolNV(device, pCreateInfo, pAllocator, pSemaphorePool);
    if (VK_SUCCESS == result) {
...
void DispatchDestroySemaphoreSciSyncPoolNV(VkDevice device, VkSemaphoreSciSyncPoolNV semaphorePool,
                                           const VkAllocationCallbacks* pAllocator) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(device), layer_data_map);
    if (!wrap_handles) return layer_data->device_dispatch_table.DestroySemaphoreSciSyncPoolNV(device, semaphorePool, pAllocator);

    uint64_t semaphorePool_id = CastToUint64(semaphorePool);
...
VkResult DispatchGetPhysicalDeviceCooperativeMatrixFlexibleDimensionsPropertiesNV(
    VkPhysicalDevice physicalDevice, uint32_t* pPropertyCount, VkCooperativeMatrixFlexibleDimensionsPropertiesNV* pProperties) {
    auto layer_data = GetLayerDataPtr(GetDispatchKey(physicalDevice), layer_data_map);

    VkResult result = layer_data->instance_dispatch_table.GetPhysicalDeviceCooperativeMatrixFlexibleDimensionsPropertiesNV(
        physicalDevice, pPropertyCount, pProperties);
