...
static inline void DispatchDestroyInstance(VkInstance instance, const VkAllocationCallbacks* pAllocator) {
    auto dispatch = vvl::GetDispatchInstance(instance);
...
static inline void DispatchGetCommandPoolMemoryConsumption(VkDevice device, VkCommandPool commandPool,
                                                           VkCommandBuffer commandBuffer,
                                                           VkCommandPoolMemoryConsumption* pConsumption) {
    auto dispatch = vvl::GetDispatchDevice(device);
    dispatch->GetCommandPoolMemoryConsumption(device, commandPool, commandBuffer, pConsumption);
}

static inline VkResult DispatchGetFaultData(VkDevice device, VkFaultQueryBehavior faultQueryBehavior, VkBool32* pUnrecordedFaults,
                                            uint32_t* pFaultCount, VkFaultData* pFaults) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->GetFaultData(device, faultQueryBehavior, pUnrecordedFaults, pFaultCount, pFaults);
}
...
static inline void DispatchCmdRefreshObjectsKHR(VkCommandBuffer commandBuffer, const VkRefreshObjectListKHR* pRefreshObjects) {
    auto dispatch = vvl::GetDispatchDevice(commandBuffer);
    dispatch->CmdRefreshObjectsKHR(commandBuffer, pRefreshObjects);
}

static inline VkResult DispatchGetPhysicalDeviceRefreshableObjectTypesKHR(VkPhysicalDevice physicalDevice,
                                                                          uint32_t* pRefreshableObjectTypeCount,
                                                                          VkObjectType* pRefreshableObjectTypes) {
    auto dispatch = vvl::GetDispatchInstance(physicalDevice);
    return dispatch->GetPhysicalDeviceRefreshableObjectTypesKHR(physicalDevice, pRefreshableObjectTypeCount,
...
static inline VkResult DispatchGetFenceSciSyncFenceNV(VkDevice device, const VkFenceGetSciSyncInfoNV* pGetSciSyncHandleInfo,
                                                      void* pHandle) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->GetFenceSciSyncFenceNV(device, pGetSciSyncHandleInfo, pHandle);
}

static inline VkResult DispatchGetFenceSciSyncObjNV(VkDevice device, const VkFenceGetSciSyncInfoNV* pGetSciSyncHandleInfo,
                                                    void* pHandle) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->GetFenceSciSyncObjNV(device, pGetSciSyncHandleInfo, pHandle);
}

static inline VkResult DispatchImportFenceSciSyncFenceNV(VkDevice device,
                                                         const VkImportFenceSciSyncInfoNV* pImportFenceSciSyncInfo) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->ImportFenceSciSyncFenceNV(device, pImportFenceSciSyncInfo);
}

static inline VkResult DispatchImportFenceSciSyncObjNV(VkDevice device, const VkImportFenceSciSyncInfoNV* pImportFenceSciSyncInfo) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->ImportFenceSciSyncObjNV(device, pImportFenceSciSyncInfo);
}

static inline VkResult DispatchGetPhysicalDeviceSciSyncAttributesNV(VkPhysicalDevice physicalDevice,
                                                                    const VkSciSyncAttributesInfoNV* pSciSyncAttributesInfo,
                                                                    NvSciSyncAttrList pAttributes) {
    auto dispatch = vvl::GetDispatchInstance(physicalDevice);
    return dispatch->GetPhysicalDeviceSciSyncAttributesNV(physicalDevice, pSciSyncAttributesInfo, pAttributes);
}

static inline VkResult DispatchGetSemaphoreSciSyncObjNV(VkDevice device, const VkSemaphoreGetSciSyncInfoNV* pGetSciSyncInfo,
                                                        void* pHandle) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->GetSemaphoreSciSyncObjNV(device, pGetSciSyncInfo, pHandle);
}

static inline VkResult DispatchImportSemaphoreSciSyncObjNV(VkDevice device,
                                                           const VkImportSemaphoreSciSyncInfoNV* pImportSemaphoreSciSyncInfo) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->ImportSemaphoreSciSyncObjNV(device, pImportSemaphoreSciSyncInfo);
}

static inline VkResult DispatchGetMemorySciBufNV(VkDevice device, const VkMemoryGetSciBufInfoNV* pGetSciBufInfo,
                                                 NvSciBufObj* pHandle) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->GetMemorySciBufNV(device, pGetSciBufInfo, pHandle);
}

static inline VkResult DispatchGetPhysicalDeviceExternalMemorySciBufPropertiesNV(
    ...
    auto dispatch = vvl::GetDispatchInstance(physicalDevice);
    return dispatch->GetPhysicalDeviceExternalMemorySciBufPropertiesNV(physicalDevice, handleType, handle, pMemorySciBufProperties);
}

static inline VkResult DispatchGetPhysicalDeviceSciBufAttributesNV(VkPhysicalDevice physicalDevice, NvSciBufAttrList pAttributes) {
    auto dispatch = vvl::GetDispatchInstance(physicalDevice);
    return dispatch->GetPhysicalDeviceSciBufAttributesNV(physicalDevice, pAttributes);
}
...
static inline VkResult DispatchCreateSemaphoreSciSyncPoolNV(VkDevice device, const VkSemaphoreSciSyncPoolCreateInfoNV* pCreateInfo,
                                                            const VkAllocationCallbacks* pAllocator,
                                                            VkSemaphoreSciSyncPoolNV* pSemaphorePool) {
    auto dispatch = vvl::GetDispatchDevice(device);
    return dispatch->CreateSemaphoreSciSyncPoolNV(device, pCreateInfo, pAllocator, pSemaphorePool);
}

static inline void DispatchDestroySemaphoreSciSyncPoolNV(VkDevice device, VkSemaphoreSciSyncPoolNV semaphorePool,
                                                         const VkAllocationCallbacks* pAllocator) {
    auto dispatch = vvl::GetDispatchDevice(device);
    dispatch->DestroySemaphoreSciSyncPoolNV(device, semaphorePool, pAllocator);
}
#endif  // VK_USE_PLATFORM_SCI
...
static inline void DispatchGetExternalComputeQueueDataNV(VkExternalComputeQueueNV externalQueue,
    ...
    auto dispatch = vvl::GetDispatchDevice(externalQueue);
