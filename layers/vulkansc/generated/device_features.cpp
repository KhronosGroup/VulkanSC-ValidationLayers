    ...
    for (const VkBaseInStructure *pNext = reinterpret_cast<const VkBaseInStructure *>(pCreateInfo->pNext); pNext != nullptr;
        ...
        switch (pNext->sType) {
            ...
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_SC_1_0_FEATURES: {
                const VkPhysicalDeviceVulkanSC10Features *enabled =
                    reinterpret_cast<const VkPhysicalDeviceVulkanSC10Features *>(pNext);
                features->shaderAtomicInstructions |= enabled->shaderAtomicInstructions == VK_TRUE;
                break;
            }
            ...
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTERNAL_SCI_SYNC_FEATURES_NV: {
                const VkPhysicalDeviceExternalSciSyncFeaturesNV *enabled =
                    reinterpret_cast<const VkPhysicalDeviceExternalSciSyncFeaturesNV *>(pNext);
                features->sciSyncFence |= enabled->sciSyncFence == VK_TRUE;
                features->sciSyncSemaphore |= enabled->sciSyncSemaphore == VK_TRUE;
                features->sciSyncImport |= enabled->sciSyncImport == VK_TRUE;
                features->sciSyncExport |= enabled->sciSyncExport == VK_TRUE;
                break;
            }
#endif  // VK_USE_PLATFORM_SCI
#ifdef VK_USE_PLATFORM_SCI
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTERNAL_MEMORY_SCI_BUF_FEATURES_NV: {
                const VkPhysicalDeviceExternalMemorySciBufFeaturesNV *enabled =
                    reinterpret_cast<const VkPhysicalDeviceExternalMemorySciBufFeaturesNV *>(pNext);
                features->sciBufImport |= enabled->sciBufImport == VK_TRUE;
                features->sciBufExport |= enabled->sciBufExport == VK_TRUE;
                break;
            }
#endif  // VK_USE_PLATFORM_SCI
            ...
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTERNAL_SCI_SYNC_2_FEATURES_NV: {
                const VkPhysicalDeviceExternalSciSync2FeaturesNV *enabled =
                    reinterpret_cast<const VkPhysicalDeviceExternalSciSync2FeaturesNV *>(pNext);
                features->sciSyncFence |= enabled->sciSyncFence == VK_TRUE;
                features->sciSyncSemaphore2 |= enabled->sciSyncSemaphore2 == VK_TRUE;
                features->sciSyncImport |= enabled->sciSyncImport == VK_TRUE;
                features->sciSyncExport |= enabled->sciSyncExport == VK_TRUE;
                break;
            }
