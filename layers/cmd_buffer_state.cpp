...
void CMD_BUFFER_STATE::Retire(uint32_t perf_submit_pass, const layer_data::unordered_set<QueryObject> &queriesUpdatedAfter) {
    ...
    for (const auto &query_state_pair : local_query_to_state_map) {
        if (query_state_pair.second == QUERYSTATE_ENDED) {
            if (queriesUpdatedAfter.find(query_state_pair.first) == queriesUpdatedAfter.end()) {
                auto query_pool_state = dev_data->Get<QUERY_POOL_STATE>(query_state_pair.first.pool);
