...
void HazardResult::AddRecordedAccess(const ResourceFirstAccess &first_access) {
    recorded_access = std::unique_ptr<const ResourceFirstAccess>(new ResourceFirstAccess(first_access));
}
...
bool CommandBufferAccessContext::ValidateFirstUse(CommandBufferAccessContext *proxy_context, const char *func_name,
    ...
    for (const auto &sync_op : sync_ops_) {
        // we include any accesses (only layout transitions) as the are stored with the first scope when recorded
    ...
    if (IsRead(usage_bit)) {
        ...
        if (last_reads.size()) {
            ...
            if ((ordered_stages & last_read_stages) != last_read_stages) {
                // For Image layout transitions, (which happen at no stage, any stage not ordered by the barrier
                // is a hazard, as the barrier represents the first synchronization scope of the layout transition
                assert((usage_index == SyncStageAccessIndex::SYNC_IMAGE_LAYOUT_TRANSITION) || (usage_stage != 0));
                const auto effective_stage = usage_stage == 0 ? ~usage_stage : usage_stage;
                for (const auto &read_access : last_reads) {
                    if (read_access.stage & ordered_stages) continue;  // but we can skip the ordered ones
                    if (IsReadHazard(effective_stage, read_access)) {
                        hazard.Set(this, usage_index, WRITE_AFTER_READ, read_access.access, read_access.tag);
                        break;
                    }
                }
    ...
    if (count) {
        ...
        if (do_write_last && tag_range.includes(last_access.tag)) {
            ...
            if (last_access.usage_index == SyncStageAccessIndex::SYNC_IMAGE_LAYOUT_TRANSITION) {
                // Or in the layout first access scope as a bbarrier... iff the usage is an ILT
...
void SyncValidator::PreCallRecordCmdPipelineBarrier(VkCommandBuffer commandBuffer, VkPipelineStageFlags srcStageMask,
    ...
    std::shared_ptr<SyncOpBase> sync_op(new SyncOpPipelineBarrier(CMD_PIPELINEBARRIER, *this, cb_access_context->GetQueueFlags(),
...
void SyncValidator::PostCallRecordCmdWaitEvents(VkCommandBuffer commandBuffer, uint32_t eventCount, const VkEvent *pEvents,
    ...
    std::shared_ptr<SyncOpBase> wait_events_op(new SyncOpWaitEvents(
