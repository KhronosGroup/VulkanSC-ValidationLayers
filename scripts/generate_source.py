...
def main(argv):
    ...
    if args.generated_version:
        ...
        if args.api == 'vulkansc':
            json_files.append(repo_relative('layers/VkSCLayer_khronos_validation.json.in'))
        else:
    ...
    # Do not use "caching" for Vulkan SC as the whole concept does not handle the fact
    # that not all generators use the same set of definitions and it is anyway unstable
