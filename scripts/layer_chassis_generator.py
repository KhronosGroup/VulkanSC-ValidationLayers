...
class LayerChassisOutputGenerator(OutputGenerator):
    ...
    # Vulkan SC validation layer list
    vksc_validation_layers = [
        {
            'include': 'thread_safety.h',
            'class': 'ThreadSafety',
    ...
    # Vulkan SC extension lists
    vksc_instance_extensions = [
        'VK_EXT_debug_utils',
        'VK_EXT_validation_features'
    ]

    vksc_device_extensions = [
        'VK_EXT_validation_cache',
        'VK_EXT_tooling_info'
    ]
...
#define BUILD_DISPATCH_VECTOR(name) \\
    ...
    vksc_init_object_dispatch_vector = """
#define BUILD_DISPATCH_VECTOR(name) \\
    init_object_dispatch_vector(InterceptId ## name, \\
                                typeid(&ValidationObject::name), \\
                                typeid(&ThreadSafety::name), \\
    ...
    def getLayerList(self):
        if self.genOpts.apiname == 'vulkansc':
            return self.vksc_validation_layers
    ...
    def getInstanceExtensions(self):
        if self.genOpts.apiname == 'vulkansc':
            return self.vksc_instance_extensions
    ...
    def getDeviceExtensions(self):
        if self.genOpts.apiname == 'vulkansc':
            return self.vksc_device_extensions
    ...
    def genInitObjectDispatchVector(self):
        if self.genOpts.apiname == 'vulkansc':
            return self.vksc_init_object_dispatch_vector
